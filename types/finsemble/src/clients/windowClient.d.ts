...
declare class WindowClient extends BaseClient {
    ...
    /**
     * This function injects the header bar into all windows without frames that request it. This should only be used if you've decided not to use the provided <code>WindowClient.start()</code> method.
     *
     * **NOTE:** If you are using the Finsemble window title bar component, you do not need to call this function.
     * @private
     */
     ...
     * @param {string} params.windowName Window whose component state you are retrieving. If null, the default is to the calling window.
     * @param {function} cb The callback to be invoked after the method completes successfully.
     ...
     * @private
     */
    getContainerHash(windowName: string): any;
    /**
     ...
     * @deprecate the union boolean type and only accept types of InjectHeaderParams.
     */
    ...
    * @private
     ...
     * This is useful to set the title dynamically based on state information: you can keep the window's title in sync with a piece of data (e.g., a Symbol).
     * This title will not persist between sessions automatically; use setComponentState/getComponentState to persist the title.
     ...
     */
    startMovingWindow(event: MouseEvent): void;
    /**
     ...
     *
     * If the calling window is not part of a stacked window, the stacked window identifier will be returned null -- unless params.create is true. In this case, a stacked window will be created and the calling window will be set as the first child
     *
     ...
     * @private
     */
    private _deriveInjectHeaderParams;
    /**
